...
import {
    FeedResponse,
    GetPaginatedFeedGateway,
    GetPaginatedFeedByTypeGateway
} from "../business/gateways/feed/feedGateway";
...
export class FeedDataBase extends BaseDataBase implements
    ...
    async getPostsFeedForUser(userId: string): Promise<FeedResponse[]>  {
        ...
        SELECT p.id, p.photo, p.description, p.type, p.creation_date as creationDate, p.userId, u.name as userName FROM users_relations_future_book rel
    ...
    async getPaginatedFeedByType(
        type: string,
        limit: number,
        offset: number,
        userId: string): Promise<FeedResponse[]> {
        const query = await this.connection.raw(`
        SELECT p.id, p.photo, p.description, p.type, p.creation_date as creationDate,
            p.userId, u.name as userName FROM users_relations_future_book rel
        JOIN posts p ON rel.requested_id=p.userId
        JOIN users_future u ON rel.requested_id=u.id
    ...
    async getPaginatedFeed(limit: number, offset: number, userId: string): Promise<FeedResponse[]> {
        const query = await this.connection.raw(`
        SELECT p.id, p.photo, p.description, p.type, p.creation_date as creationDate,
            p.userId, u.name as userName FROM users_relations_future_book rel
        JOIN posts p ON rel.requested_id=p.userId
